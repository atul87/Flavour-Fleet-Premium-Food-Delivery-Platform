...
def should_expose_dev_reset_details():
    explicit_flag = os.getenv("ALLOW_DEV_RESET_CODE")
    if explicit_flag is not None:
        return explicit_flag.lower() in {"1", "true", "yes"}
    # Secure-by-default: only expose reset codes when explicitly enabled.
    return False
    ...
    if guest_id:
        ...
        if guest_cart and guest_cart.get("items"):
            ...
            if user_cart and user_cart.get("items"):
                ...
                carts_col.update_one(
                    {"user_id": user_id}, {"$set": {"items": user_cart["items"]}}
                )
            else:
                carts_col.update_one(
                    {"user_id": user_id},
...
def get_profile():
    if "user_id" not in session:
        return (
            jsonify({"success": False, "logged_in": False, "message": "Not logged in"}),
            200,
        )
